...
字段如下：
{
  ...
  "food": ["如果有食物，列出具体食物名称；如果没有，返回空数组"],
  "objects": ["图中明显物体，尽量具体到可用于日记的名词"],
  "landmark_or_place_hint": "结合招牌、文字、建筑、环境判断地点线索；不确定则写 unknown",
  "fun_fact": "基于图中能看到的真实内容，写一句有趣的科普或描述（如菜系特点、建筑风格、食材文化等）。必须是图中可见的，不要编造看不见的信息。不确定则写空字符串。",
  "confidence": "high / medium / low",
  "diary_sentence": "用中文写一句自然的旅行日记描述"
}

注意：
1. 不要做人脸身份识别，不要猜测具体人物是谁
2. 不要输出照片氛围/心情类描述（如 happy、relaxed 等）
3. 如果不确定，用 unknown 或 confidence: low
4. 如果图中有可读文字（店名、菜单、路牌等），把它融合进 objects 或 landmark_or_place_hint
5. fun_fact 必须基于图中可见内容，不可凭空编造
6. 输出必须是合法 JSON object"""
    ...
    scenarios = [
        {
            ...
            "food": ["云吞面", "丝袜奶茶"],
            "objects": ["餐桌", "碗碟", "筷子", "菜单"],
            "landmark_or_place_hint": "港式茶餐厅",
            "fun_fact": "云吞面是香港经典平民美食，鲜虾云吞配竹升面，汤头用大地鱼熬制。",
            "confidence": "medium",
            "diary_sentence": "这张照片看起来是在一家港式茶餐厅用餐，桌上有云吞面和奶茶。",
        },
        {
            ...
            "food": [],
            "objects": ["建筑", "天空", "游客", "地标"],
            "landmark_or_place_hint": "城市地标建筑",
            "fun_fact": "这座建筑的玻璃幕墙设计采用了双层中空结构，能有效隔热降噪。",
            "confidence": "medium",
            "diary_sentence": "这张照片看起来是在一个城市地标参观。",
        },
        {
            "scene_type": "street",
            "activity": "walking",
            "food": [],
            "objects": ["街道", "商铺", "行人", "招牌"],
            "landmark_or_place_hint": "商业街区",
            "fun_fact": "霓虹招牌是香港街头文化的标志，曾是城市夜景的重要组成部分。",
            "confidence": "medium",
            "diary_sentence": "这张照片看起来是在繁华的商业街区漫步。",
        },
        {
            "scene_type": "beach",
            "activity": "relaxing",
            "food": [],
            "objects": ["海", "沙滩", "遮阳伞", "天空"],
            "landmark_or_place_hint": "海岸线",
            "fun_fact": "海滩的沙质看起来是珊瑚砂，常见于热带海岸，由珊瑚碎屑经海浪冲刷形成。",
            "confidence": "medium",
            "diary_sentence": "这张照片看起来是在海边，沙滩和海浪构成了一幅宁静的画面。",
        },
        {
            "scene_type": "museum",
            "activity": "sightseeing",
            "food": [],
            "objects": ["展品", "艺术品", "展柜", "展厅"],
            "landmark_or_place_hint": "博物馆展厅",
            "fun_fact": "博物馆的灯光色温通常控制在 3000K 左右，以兼顾展品保护和观赏效果。",
            "confidence": "medium",
