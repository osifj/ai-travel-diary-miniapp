...
如果 MiMo 的 API 格式与 OpenAI 不一致，请根据官方文档修改 _call_api 方法。

配置方式:
  环境变量:
    MIMO_API_KEY   - API Key
    MIMO_BASE_URL  - API Base URL (e.g. https://api.mimo.ai/v1)
...
# ---- AI Prompt ----
ANALYSIS_PROMPT = """你是一个旅行照片分析助手。请分析这张照片，但不要识别人脸身份，不要判断具体人物是谁。
...
字段如下：
{
  ...
  "objects": ["图中明显物体"],
  "landmark_or_place_hint": "如果有明显地标或地点线索，请写出；不确定则写 unknown",
...
def _call_api(image_path: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError("MiMo API 请求超时 (60s)")
